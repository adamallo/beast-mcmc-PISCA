/*
 * SequenceErrorLikelihoodCore.java
 *
 * Provisional, by DM
 * 
...
public class SequenceErrorLikelihoodCore extends AbstractLikelihoodCore {
	 ...
	 */
	public SequenceErrorLikelihoodCore(int stateCount) {
		super(stateCount);
	}
			...
			for (int k = 0; k < patternCount; k++) {
                ...
                if (state1 < stateCount && state2 < stateCount) {
					...
					for (int i = 0; i < stateCount; i++) {
                        ...
                        // Debugging:
                        //partials3[v] = matrices1[w + state1] * matrices2[w + state2];
                        // Added sequencing error
                        if(state1 == 0 && state2 == 0){
                                partials3[v] = ((1-seqError2)*matrices1[w + state1]+seqError2*matrices1[w + 1])*
                                        ((1-seqError2)*matrices2[w + state2]+seqError2*matrices1[w + 1]);
                        } else if(state1 == 1 && state2 == 1){
                                partials3[v] = ((1-seqError1)*matrices1[w + state1]+seqError1*matrices1[w + 0])*
                        ((1-seqError1)*matrices2[w + state2]+seqError1*matrices1[w + 0]);
			...
			for (int k = 0; k < patternCount; k++) {
				...
				if (state1 < stateCount) {
					...
					for (int i = 0; i < stateCount; i++) {
                        ...
                        if (state1==0){
                            tmp = (1-seqError2)*matrices1[w + state1]+seqError2*matrices1[w + 1];
                        } else {
                            tmp = (1-seqError1)*matrices1[w + state1]+seqError1*matrices1[w + 0];
                        }
